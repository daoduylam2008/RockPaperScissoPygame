...
class MenuView:
    ...
    def __init__(self, surface, width, height):
        ...
        self.groupWidget.widgets.extend([self.button_singleplay, self.button_multiplay, self.button_setting, self.button_quit, self.inputBox])
...
class SinglePlayerView:
    ...
    def __init__(self, surface, width, height, back, who_win, input_text):
        ...
        self.buttons = {name: self.create_buttons(surface,name.capitalize(),action) for name,action in self.button_action.items()}
        ...
        self.backgroundImageSinglePlayer = pygame.transform.scale(self.backgroundImageSinglePlayer, (int(self.width), int(self.height)))

    
    def create_buttons(self,surface,text,action,x_offset = 0):
        match text:
            case 'Rock': x_offset = 300
            case 'Scissors': x_offset = 200
            case 'Paper': x_offset = 100
        button = uix.Button(surface,rect = (surface.get_rect().bottomright[0]- x_offset,surface.get_rect().bottomright[1] - 80,90,50),text=text,color = '#ff6680',bottom_rect_color='#ed7700',text_color='black',text_size=23)
...
class Settings:
    ...
    def resize(self):
        ...
        self.groupWidget_resize.widgets.extend([resizeSmallButton, resizeMediumButton, resizeLargeButton, resizeFullScreenButton, resizeBackButton])
