...
class SinglePlayerView:
	def __init__(self, surface, back, who_win, input_text):
...
class SelectClientServerView(uix.Widget):
	def __init__(self, surface):
		...
		self.client = ClientView(self.surface)
...
class Settings:
	def __init__(self, surface, func_back, SettingsView):
	...
	def resize(self):
		...
		resizeFullScreenButton = uix.Button(self.surface, (
			self.surface.get_rect().center[0] - 80, self.surface.get_rect().center[1] + 150, 150, 60),
											text='Full Screen',
											color=(204, 204, 196), bottom_rect_color=(255, 255, 255),
											text_color='#FFFF00')
...
class RockPaperScissor:
	def __init__(self):
		...
		self.singlePlayerView = SinglePlayerView(self.screen, self.back, self.who_win, self.menuView.inputBox.text)

		# The Multi player View
		self.multiPlayerView = MultiPlayerView(self.screen)

		# The Settings View
		self.settingsView = Settings(self.screen, self.back, self.SetingsView)
